  ...
  .controller('TelecomTelephonyLinePhoneProgammableKeysEditCtrl', function (
    ...
    * */
    ...
    function getFaxParameters() {
      return $q.all([
        OvhApiTelephonyFax.Aapi().getServices().$promise.then((faxList) => {
    ...
    function getHuntingParameter() {
      return TelephonyMediator.getGroup($stateParams.billingAccount).then((curGroup) => {
        const pabxState = {
    ...
    self.setParameters = function () {
      ...
      return request.then((availableParameters) => {
    ...
    const init = function init() {
      ...
      return self.FunctionKey.getAvailableFunctions().then((availableFunctions) => {
