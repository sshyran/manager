...
angular.module('managerApp').controller(
  'TelecomTelephonyLinePhoneOrderCtrl',
  function TelecomTelephonyLinePhoneOrderCtrl(
    $q,
    $scope,
    $stateParams,
    $translate,
    ...
    function fetchMerchandiseAvailable() {
      return OvhApiTelephony.Line().Phone().v6().getMerchandiseAvailable({
        ...
        forEach(result, (phone) => {
          ...
          set(phone, 'displayName', `${capitalize(head(parts))} ${parts.slice(1).map(p => (p || '').toUpperCase()).join(' ')}`);
        });
