...
angular.module('managerApp').factory('TelecomTelephonyLineCallsForwardNature', ($translate) => {
   ...
   */
  const TelecomTelephonyLineCallsForwardNature = function TelecomTelephonyLineCallsForwardNature(elt) { // eslint-disable-line
    this.label = $translate.instant(`telephony_line_actions_line_calls_forward_${elt}`);
    this.value = elt;
    switch (elt) {
      case 'number':
