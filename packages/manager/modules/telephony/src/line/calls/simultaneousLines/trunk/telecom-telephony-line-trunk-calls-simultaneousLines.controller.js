...
angular.module('managerApp').controller('TelecomTelephonyLineTrunkSimultaneousLines', class TelecomTelephonyLineTrunkSimultaneousLines {
  ...
  _getNewRepartition() {
    if (this.trunkRates.change.quantityWanted > 0
      ...
      return this.OvhApiTelephony.Trunk().v6().getChannelsPacksRepartition({
        ...
        this.trunkRates.change.price = packResult.totalPrice.text + this.$translate.instant('telephony_line_actions_line_calls_trunk_simultaneous_channels_price_per_month');
        this.trunkRates.change.packsRepartition = packResult.packsRepartition;
        this.trunkRates.change.quantityOptimized = packResult.optimizedChannelsQuantity;
  ...
  calculateBandwidth() {
    ...
    this.trunkRates.change.bandwidth = bandwidthRate * this.trunkRates.change.quantityOptimized;
  }
    ...
    return this.OvhApiTelephony.Trunk().v6().getChannelsPacksRepartition({
      ...
      if (this.currentLine.simultaneousLinesDetails.toBeDeleted > 0) {
        return this.OvhApiTelephony.Trunk().v6().getChannelsPacksRepartition({
          ...
          this.trunkRates.pending.price = pendingPack.totalPrice.text + this.$translate.instant('telephony_line_actions_line_calls_trunk_simultaneous_channels_price_per_month');
          this.trunkRates.pending.packsRepartition = pendingPack.packsRepartition;
