...
export default /* @ngInject */ function ($q, $stateParams, $timeout, $translate,
  OvhApiOrder, TucToast) {
  const self = this;
  ...
  self.orderConvertToVoicefax = function orderConvertToVoicefax() {
    ...
    }, pick(self.convertToVoicefaxForm, 'billingAccount')).$promise.then((order) => {
      self.convertToVoicefaxForm.prices = order;
    }).catch((err) => {
      TucToast.error([$translate.instant('telephony_service_fax_convert_to_voicefax_list_error_ordering'), get(err, 'data.message', '')].join(' '));
    }).finally(() => {
  ...
  function init() {
    ...
    return fetchConvertToVoicefaxInformations().then((services) => {
      ...
      TucToast.error([$translate.instant('telephony_service_fax_convert_to_voicefax_list_error_loading'), get(err, 'data.message', '')].join(' '));
    }).finally(() => {
      self.isLoading = false;
    });
  }
