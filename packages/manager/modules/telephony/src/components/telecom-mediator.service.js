...
export default /* @ngInject */ function ($q, $translate, OvhApiMeVipStatus,
  TucPackMediator, TelephonyMediator, TucSmsMediator,
  TucFaxMediator, TucOverTheBoxMediator, TucToast) {
  const self = this;
    ...
    $q.all({
      ...
      if (countErrors.length) {
        $translate.refresh().then(() => {
          TucToast.error(`${$translate.instant('sidebar_init_error')}<br/><br/>${countErrors.join('<br />')}`, {
            hideAfter: false,
          });
        });
      }
