  ...
  return {
    ...
    link(scope, element) {
      ...
      function loadAssets() {
        ...
        const template = `${getNumberType(scope.number) + (fill ? `_${fill}` : '')}.png`;
        return $q.all({
      ...
      function renderNumber(number, fontImage, fontAdvance) {
        ...
        forEach(number, (c) => {
          if (c < '0' || c > '9') {
            if (c === ',' || c === '.') {
              ctx.drawImage(
                fontImage, fontAdvance * 10, 0, fontAdvance, fontImage.height,
            ...
            ctx.drawImage(
              fontImage, fontAdvance * i, 0, fontAdvance, fontImage.height,
              pos, 0, fontAdvance, fontImage.height,
            );
            pos += fontAdvance;
          }
      ...
      function renderSva(number, assets) {
        ...
        let numberCanvas = renderNumber(formattedNumber, assets.font, scope.scale.fontAdvance);
        if (scope.fill !== 'black') {
        ...
        // draw price
        if (price) {
          let priceCanvas = renderNumber(`${price.value}`, assets.fontSmall, scope.scale.fontSmallAdvance);
          ...
          ctx.drawImage(
            ...
            priceCanvas.width - (scope.scale.fontSmallAdvance / 2) + 2 + pricePos[0], pricePos[1], // eslint-disable-line
          );
        }
