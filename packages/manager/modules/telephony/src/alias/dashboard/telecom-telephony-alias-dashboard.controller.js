...
angular.module('managerApp').controller('TelecomTelephonyAliasDashboardController', class TelecomTelephonyAliasHomeController {
  constructor(
    ...
    TELEPHONY_REDIRECT_URLS, TELEPHONY_ALIAS_CONSUMPTION, TELEPHONY_ALIAS_OBSOLETE_FEATURE_TYPES,
  ) {
    this.$q = $q;
    this.$state = $state;
    ...
    this.TELEPHONY_REDIRECT_URLS = TELEPHONY_REDIRECT_URLS;
  ...
  buildConsumptionChart(incomingCalls, outgoingCalls) {
    ...
    let xAxisKeys = union(_outgoingCalls, _incomingCalls);
    xAxisKeys = sortBy(xAxisKeys, 'callDate');
    xAxisKeys = groupBy(xAxisKeys, 'callDate');
    xAxisKeys = keys(xAxisKeys);
