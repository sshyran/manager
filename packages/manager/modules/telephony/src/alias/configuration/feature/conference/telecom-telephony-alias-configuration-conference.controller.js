...
angular.module('managerApp').controller('TelecomTelephonyAliasConfigurationConferenceCtrl', class TelecomTelephonyAliasConfigurationContactCenterSolutionCtrl {
  ...
  $onInit() {
    ...
    this.loading = true;
    return this.tucVoipServiceAlias.fetchConferenceNumber(this.serviceInfos)
      .then((conference) => {
        ...
        return this.tucVoipServiceAlias.fetchConferenceNumberWebAccess(this.serviceInfos);
      })
      .then((webAccess) => {
  ...
  checkValidAudioExtention({ name }) {
    ...
    const found = this.TELEPHONY_SERVICE.validSoundExtensions.includes(fileExtension);
    if (!found) {
      this.TucToast.error(this.$translate.instant('telephony_alias_config_conference_invalid_file_format'));
    }
    return found;
  }
  ...
  deleteWebAccess() {
    ...
    return this.$q.all(
      this.webAccess.followUp
        ? this.tucVoipServiceAlias.deleteConferenceNumberWebAccess(this.serviceInfos, {
  ...
  updateConference() {
    ...
    return uploadFilePromise.then((fileUploaded) => {
      if (fileUploaded) {
      ...
      this.TucToast.success(this.$translate.instant('telephony_alias_config_conference_update_success'));
      this.$onInit();
    }).catch((error) => {
      this.TucToast.error(
