...
angular.module('managerApp').controller('TelecomTelephonyAliasConfigurationLinesEditCtrl', class TelecomTelephonyAliasConfigurationLinesEditCtrl {
  constructor(
    ...
    enumStatus, lineToEdit, tucVoipServiceAlias,
  ) {
  ...
  updateLine() {
    this.loading = true;
    const {
      status, simultaneousLines, timeout, wrapUpTime,
    } = this.lineToEdit;

    return this.tucVoipServiceAlias.updateContactCenterSolutionNumberAgent({
    ...
    this.lineToEdit.agentId,
    {
      ...
      this.$uibModalInstance.dismiss(
        this.$translate.instant('telephony_alias_config_contactCenterSolution_lines_line_edit_error'),
      );
    }).finally(() => {
      this.loading = false;
    });
