...
angular.module('managerApp').controller('TelecomTelephonyAliasConfigurationCallsFilteringCtrl', class TelecomTelephonyAliasConfigurationCallsFilteringCtrl {
  ...
  $onInit() {
    ...
    this.loading = true;
    return this.$q.all({
      ccsNumber: this.tucVoipServiceAlias
        .fetchContactCenterSolutionNumber(this.serviceInfos),
      screenLists: this.tucVoipServiceAlias
        .fetchContactCenterSolutionNumberScreenListsConditions(this.serviceInfos),
  ...
  fetchScreenListsStatus() {
    return this.tucVoipServiceAlias
      .fetchContactCenterSolutionNumberScreenListsStatus(this.serviceInfos);
  }
  ...
  addScreenList(screen) {
    ...
    return this.tucVoipServiceAlias.addContactCenterSolutionNumberScreenListCondition(screenToAdd);
  }

  deleteCondition(conditionToRemove) {
    this.deleting = true;
    return this.deleteScreenListCondition(conditionToRemove)
      .then(() => this.tucVoipServiceAlias
        .fetchContactCenterSolutionNumberScreenListsConditions(this.serviceInfos))
      .then((screenLists) => {
        this.screenLists = this.formatScreenLists(screenLists);
      })
  ...
  deleteScreenListCondition(conditionToRemove) {
    return this.tucVoipServiceAlias
      .removeContactCenterSolutionNumberScreenListConditions(this.serviceInfos, conditionToRemove);
  }
  ...
  updateScreenListConditionsConfiguration() {
    ...
    return this.$q.all({
      addConditions: this.canAddConditions() ? this.addConditions() : this.$q.when(),
      updateAnonymousRejections: this.canUpdateAnonymousRejection() ? this.tucVoipServiceAlias
        .updateContactCenterSolutionNumber(
          this.serviceInfos,
          this.number,
        ) : this.$q.when(),
      updateScreenListFiltering: this.tucVoipServiceAlias
      ...
      .then(() => this.$state.go('^').then(() => {
        this.TucToast.success(this.$translate.instant('telephony_alias_configuration_calls_filtering_add_success'));
      }))
      .catch((error) => {
        this.TucToast.error(
  ...
  deleteConditions(conditions) {
    this.$uibModal.open({
      templateUrl: 'app/telecom/telephony/alias/configuration/feature/contactCenterSolution/callsFiltering/delete/telecom-telephony-alias-configuration-calls-filtering-delete.html',
      controller: 'TelecomTelephonyAliasConfigurationCallsFilteringDeleteCtrl',
      controllerAs: '$ctrl',
      resolve: {
  ...
  onBulkSuccess() {
    return (bulkResult) => {
      this.tucTelephonyBulk.getTucToastInfos(bulkResult, {
        fullSuccess: this.$translate.instant('telephony_line_calls_filtering_bulk_all_success'),
        partialSuccess: this.$translate.instant('telephony_line_calls_filtering_bulk_some_success', {
          count: bulkResult.success.length,
        }),
        error: this.$translate.instant('telephony_line_calls_filtering_bulk_error'),
      }).forEach((toastInfo) => {
