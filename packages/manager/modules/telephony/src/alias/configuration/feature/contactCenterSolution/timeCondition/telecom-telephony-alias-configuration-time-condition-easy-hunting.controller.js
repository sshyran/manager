...
angular.module('managerApp').controller('TelecomTelephonyAliasConfigurationTimeConditionCtrl', class TelecomTelephonyAliasConfigurationTimeConditionCtrl {
  ...
  $onInit() {
    this.loading = true;
    this.timeConditionLabel = this.$translate.instant('telephony_alias_config_contactCenterSolution_timeCondition_label').toLowerCase();
  ...
  exportConfiguration() {
    if (this.number.feature.timeCondition.conditions) {
      this.voipTimeConditionConfiguration
        .exportConfiguration(this.number.feature.timeCondition.conditions);
    }
  }
  ...
  importConfiguration() {
    this.$uibModal.open({
      ...
      .then((conditionsToImport) => {
        // Set existing condition state to delete
        this.number.feature.timeCondition.conditions = this.number.feature.timeCondition.conditions
          .map((condition) => {
        ...
        return this.number.feature.timeCondition.saveConditions().then(() => {
          this.number.feature.timeCondition.conditions = this.number.feature.timeCondition
            .conditions.concat(
  ...
  hasChanged() {
    const isConditionsInEdition = this.number.feature.timeCondition
      .conditions.some(({ inEdition }) => inEdition);
    return !isConditionsInEdition && this.number.feature.timeCondition.hasChange();
  }
  ...
  updateContactCenterSolution() {
    ...
    // first save options
    return this.number.feature.timeCondition.save()
      .then(() => {
  ...
  onBulkSuccess() {
    return (bulkResult) => {
      ...
      this.tucTelephonyBulk.getTucToastInfos(bulkResult, {
        fullSuccess: this.$translate.instant('telephony_line_calls_time_condition_bulk_all_success'),
        partialSuccess: this.$translate.instant('telephony_line_calls_time_condition_bulk_some_success', {
          count: bulkResult.success.length,
        }),
        error: this.$translate.instant('telephony_line_calls_time_condition_bulk_error'),
