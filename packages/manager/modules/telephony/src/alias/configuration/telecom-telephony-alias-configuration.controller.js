...
angular.module('managerApp').controller('TelecomTelephonyAliasConfigurationCtrl', class TelecomTelephonyAliasConfigurationCtrl {
  constructor($q, $stateParams, $translate,
    atInternet, TelephonyMediator, TucToast) {
    this.$q = $q;
  ...
  $onInit() {
    ...
    return this.TelephonyMediator
      ...
      .then((number) => {
        ...
        this.TucToast.error(
          `${this.$translate.instant('telephony_alias_configuration_load_error')} ${get(error, 'data.message', '')}`,
        );
      })
      .finally(() => {
        this.loading = false;
  ...
  getFeatureTypeActions() {
    ...
    if (this.number.getFeatureFamily() === 'ovhPabx') {
      ...
      if (this.number.feature.featureType !== 'cloudIvr') {
        const isContactCenterSolutionExpert = this.number.feature.featureType === 'contactCenterSolutionExpert';
