...
angular.module('managerApp').controller('TelecomTelephonyAliasStatisticsCtrl', class TelecomTelephonyAliasStatisticsCtrl {
  ...
  getQueueId() {
    ...
    return this.tucVoipServiceAlias.fetchContactCenterSolutionNumberQueues(this.serviceInfos)
      .then((queues) => {
        [this.queue] = queues;
      });
  }
  ...
  refreshStats() {
    return this.$q.all({
      agentsStatus: this.tucVoipServiceAlias
        .fetchContactCenterSolutionNumberAgentsStatus(this.serviceInfos, this.queue.queueId),
      calls: this.tucVoipServiceAlias
        .fetchContactCenterSolutionNumberQueueCalls(this.serviceInfos, this.queue.queueId),
      stats: this.tucVoipServiceAlias
        .fetchContactCenterSolutionNumberQueueStatistics(this.serviceInfos, this.queue.queueId),
    })
  ...
  getQoS() {
    ...
    return Math.round((this.stats.callsAnswered / this.stats.callsTotal) * 10000) / 100;
  }
  ...
  getOnCallAgentsCount() {
    return this.agentsStatus.filter(({ status }) => ['inAQueueCall', 'receiving'].includes(status)).length;
  }

  getWaitingAgentsCount() {
    return this.agentsStatus.filter(({ status }) => status === 'waiting').length;
  }
  // End of helpers
